...
var app = new Vue({
	...
	watch: {
		...
		itemcount: function(val, oldval) {
			...
			if (val > 2) {
				...
				if (this.parentheses.min > this.parentheses.max) this.parentheses.max = this.parentheses
					.max;
			} else {
	...
	methods: {
		...
		genItem: function() {
			...
			var w = '' === this.whichcond ? randomInt(0, this.itemcount - 1) : this.whichcond -
				0; // 已知得数，随机求某一个条件
			var min = range[0].min,
				max = range[0].max,
				limit = [],
			...
			if ('+' == op) {
				...
				rg1 = {
					min: result.min - range[1].min,
					max: result.max - range[1].min
				};
			} else if ('-' == op) {
				// 减法：根据结果和减数的限制范围，确定被减数的范围，确保能在最小的减数上能产生借位
				if ('all' == this.borrow) {
					min = Math.max(min, (range[1].min - 0) + (result.min - 0) +
						10); // 强制借位时，必须比减数至少大 10 以上才行!
				} else {
					min = Math.max(min, (range[1].min - 0) + (result.min - 0)); // 被减数不允许比(减数+得数)还小，这会产生负数结果
				}
				rg1 = {
					min: (result.min - 0) + (range[1].min - 0),
					max: (result.max - 0) + (range[1].min - 0)
				};
			} else if ('*' == op) {
			...
			for (var i = 1; i < this.itemcount; i++, op = this.op()) {
				...
				var rgc = {
					min: eval(r + op + range[i].min),
					max: eval(r + op + range[i].max)
				};
		...
		getItemUserLORRange: function(operator, lor) {
			...
			return {
				min: item_user_range.min - 0,
				max: item_user_range.max - 0
			};
		},
		...
		calcItemLORRange: function(items, item, lor, is_calc_by_result) {
			...
			if ('+' == other.operator) {
				...
				max = (0 == other_range.min) ? other.result : this.calcItem2Value(items, other, other_range
					.min);
			} else if ('/' == other.operator) {
				// 除法时，且结果不为 0 的情况，
			...
			console.log(Array(item.index * 4).fill('-').join('') + 'calcItemLORRange(' + item.index + ', ' +
				item.lor + ')：', '运算符：', item.operator, '，结果：', item.result,
				'，用户设置范围：', item_user_range.min + '~' + item_user_range.max,
				'，子层结果范围：', (child_result_range ? child_result_range.min + '~' + child_result_range
					.max : ''),
		...
		getItemUserResultRange: function(operator) {
			...
			return {
				min: user_result_range.min - 0,
				max: user_result_range.max - 0
			};
		},
		...
		calcItemResultRange: function(items, item) {
			...
			if ('+*'.indexOf(item.operator) >= 0) {
				...
				max = (0 == right_range.min) ? left_range.max : Math.round(left_range.max / right_range
					.min);
			}

			var inter_range = intersectionRange({
				min: min - 0,
				max: max - 0
			}, user_result_range);
			...
			if (!inter_range) {
				...
				console.log(Array(item.index * 4).fill('-').join('') + 'calcItemResultRange(' + item.index +
					', ' + parent_lor + ')：', '运算符：', item.operator,
		...
		genItemResult: function(items, item, parent) {
			...
			var result, min = result_range.min,
			...
			// 减法强制借位：
				...
				if (item.index > 0 && 'yes' == this.nomod && '*' == items[item.index - 1].operator) {
					...
					if (inarr.length <= 0) {
						setError("    错误：在第", item.index, '层找不到能被父层结果', items[item.index - 1].result,
							'除尽的数值');
					}
				}
				// 如果父层是除法，本层在父层是作为被除数，而且父层已有结果时，要参考父层结果来修正
				else if (item.index > 0 && 'yes' == this.nomod && '/' == items[item.index - 1].operator &&
					items[item.index - 1].lor == 'lft') {
					inarr = [];
			...
			// 非顶层要考虑本层和父层两个范围的合并问题：
				...
				var inter_range = intersectionRange(parent_lor_range, {
					min: min - 0,
					max: max - 0
				});
				if (!inter_range) {
					// 此时只能尽量满足范围小的一层的要求，以小的范围为准？
					setWarning('    警告：父层左|右值范围与当前层结果范围无交集：', JSON.stringify(parent_lor_range), JSON
						.stringify(result_range));
				} else {
					min = inter_range.min;
					max = inter_range.max;
				...
				result = this.genItemLORByRange(items, parent, {
					min: min,
					max: max,
					genotinarr: genotinarr,
					notinarr: notinarr,
			...
			console.log(Array(item.index * 4).fill('-').join('') + 'genItemResult(' + item.index + ', ' +
				item.lor + ')：',
				(item.lor == 'lft' ? '?' : 'X') + ' ' + item.operator + ' ' + (item.lor == 'rgt' ? '?' :
					'X') + ' = ', result, '(' + result + '=' + min + '~' + max + ')');
		...
		genItemLOR: function(items, item) {
			...
			if (isNaN(val)) {
				setError(Array(item.index * 4).fill('-').join('') + 'genItemLOR error:', val, JSON
					.stringify(item));
			} else {
				console.log(Array(item.index * 4).fill('-').join('') + 'genItemLOR(' + item.index + ', ' +
					item.lor + ')：',
					(item.lor == 'lft' ? val : 'X') + ' ' + item.operator + ' ' + (item.lor == 'rgt' ?
						val : 'X') + ' = ' + item.result, '(' + val + '=' + range.min + '~' + range
					.max + ')');
			}
			return val;
		},
		...
		genOneFormula: function(items, index) {
			...
			if (real_result != items[index].result) {
				setWarning('    第', index, '层结果与之前随机生成的结果', items[index].result, '不一致，已经重新设置为新结果：',
					real_result);
		...
		formulaToString: function(items) {
			...
			for (var i = items.length - 1; i >= 0; i--) {
				...
				if (i > 0 && this.parentheses.autofix) {
					// 自动去掉无意义的括号
					if ('+*'.indexOf(items[i].operator) >= 0 && items[i].operator == items[i - 1]
						.operator) {
						head = tail = '';
					} else if ('+-'.indexOf(items[i - 1].operator) >= 0 && '*/'.indexOf(items[i]
							.operator) >= 0) {
						head = tail = '';
					} else if ('-' == items[i - 1].operator && '-' == items[i].operator && 'lft' == items[
							i - 1].lor) {
						head = tail = '';
					} else if ('/' == items[i - 1].operator && '/' == items[i].operator && 'lft' == items[
							i - 1].lor) {
				...
				if (i == items.length - 1) {
					if ('2' == this.rule && w == i + 1) tmpstr = items[i].lft + op + this.blank(items[i]
						.rgt);
					else if ('2' == this.rule && w == i + 0) tmpstr = this.blank(items[i].lft) + op + items[
						i].rgt;
			...
			// console.log("strstr,", str.slice(0,-3), items[0].result);
			ans_formula.push({
				"forr": str.slice(0,-3),
				"ans": items[0].result
			});
		...
		doGen: function() {
			...
			for (var i = 0; i < this.count; i++) {
				...
				this.res.push(item);
				// console.log("dasdasdasdasd", item.li);
			}
		...
		doExportWord: function() {
			// $("button[class='wordbutton']").click(function(event) {
			// 	$(".word").wordExport('word文档');
			// });
			$("#table-forr").wordExport($('#leader_name').text()+'word文档');
			//ganburenmianbaio为需要导出的内容的元素ID，参数是导出的word文档的名称
		},
		doExamination: function(){
			// isvisual = true;
			// document.body.remove();
			get_result()
			location.href = "result.html";
			// root.parentNode.removeChild(div); 
		},
		...
		doEditData: function(obj_id) {
			...
			query.get(obj_id, {
				success: function(object) {
					...
					object.save(null, {
						success: function(objectUpdate) {
							alert("create object success, object score:" +
								objectUpdate.get("score"));
						},
						error: function(model, error) {
							alert("create object fail");
						}
